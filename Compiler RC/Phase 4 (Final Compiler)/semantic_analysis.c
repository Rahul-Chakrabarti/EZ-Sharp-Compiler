// By Rahul Chakrabarti and Ethan Sadler
// semantic + if semantically correct generate 3TAC code
...
int recursion(ASTNode* root, int curr, int temp){
    int errrrr = 0;
    ...
    if (root->children[curr]->searched == 0 && root->children[curr]->type == 1){ //if the leftmost child node hasn't been searched and is a non-terminal
       ...
       recursion(create(root->children[curr], 0, ltable[l1x][l1y]-1), 0, temp);
        ...
        if (my_strcmp(root->children[curr]->name, term) == 0){
            ...
            if (inter_flag == 1 && !(my_strcmp(tokenpair[x-1][1], "then") == 0) && !(my_strcmp(tokenpair[x-1][1], "do") == 0)){
                printf("%d, %d type bad\n", inter_line, my_atoi(tokenpair[x][2]));
